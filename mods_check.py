...
def get_profile(oauth_headers, membership_id):
    profile_uri = "{0}Destiny2/3/Profile/{1}/?components={2}".format(
        api.api_url_base, membership_id, "100"
    )
...
def get_collections(oauth_headers, membership_id):
    profile_uri = "{0}Destiny2/3/Profile/{1}/?components={2}".format(
        api.api_url_base, membership_id, "800"
    )
    response = requests.get(profile_uri, headers=oauth_headers)
    if response.status_code == 200:
        response_object = json.loads(response.content.decode("utf-8"))
...
def get_vendor(oauth_headers, membership_id, character_id, vendor_hash):
    vendors_uri = (
        "{0}Destiny2/3/Profile/{1}/Character/{2}/Vendors/{3}/?components={4}".format(
            api.api_url_base, membership_id, character_id, vendor_hash, "402"
        )
    )
...
def get_item_definition(hashidentifier):
    definition_uri = "{0}Destiny2/Manifest/DestinyInventoryItemDefinition/{1}/".format(
        api.api_url_base, hashidentifier
    )
...
def item_search(search_term):
    search_uri = "{0}Destiny2/Armory/Search/DestinyInventoryItemDefinition/{1}/".format(
        api.api_url_base, search_term
    )
    response = requests.get(search_uri, headers=api.api_key_headers)
    if response.status_code == 200:
        response_object = json.loads(response.content.decode("utf-8"))
...
def banshee_has_new_mod(oauth_headers, membership_id):
    ...
    banshee_vendor_info = get_vendor(
        oauth_headers, membership_id, first_character_id, "672118013"
    )
