...
class FomoBotClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if (message.content.startswith('!mods')):
            ...
            item_found = mods_check.item_search(pattern.search(message.content).group(1))
            ...
            damage_type = DestinyEnums.DamageType(item_definition['Response']['defaultDamageType']).name
            ammo_type = DestinyEnums.DestinyAmmunitionType(item_definition['Response']['equippingBlock']['ammoType']).name
            ...
            embed.add_field(name="light.gg", value=reply_strings.make_light_gg_link(item_found))
