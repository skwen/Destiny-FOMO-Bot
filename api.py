...
def get_headers(user_oauth_code):
    ...
    response = requests.post(token_uri,headers=api_key_headers, data={'grant_type': 'authorization_code', 'code': user_oauth_code, 'client_id': auth_tokens.client_id, 'client_secret': auth_tokens.client_secret})
    if response.status_code == 200:
        response_object = json.loads(response.content.decode('utf-8'))
        access_token = response_object['access_token']
        headers = {'X-API-Key': auth_tokens.api_key, 'Authorization': 'Bearer {0}'.format(access_token)}
        return headers
    else:
        print('Oauth Error')
